...
/**
 * Description of ref_acc_number_mdl
 *
 * @author ino
 */
class Ref_Acc_Number_Mdl extends Apppayroll_Frontmdl {

    public $payslip_mdl_inst              = 'payslip_mdl';
    public $tbl                           = 'apr_ref_acc_number';
    ...
    public $rs_use_form_filter            = 'apr_ref_acc_number';
...
//        }
        ...
        if ($use_filter) {
            $this->db->where('id_pegawai NOT IN (SELECT `empl_id` FROM `apr_ref_acc_number` WHERE `active_status`=1) ', null, false);
        }
        $rs = $this->db->get();
        if ($rs->num_rows() <= 0) {
            return array();
    ...
    public function add_new_benefactor($empl_id, $eff_date, $term_date,$text) {
        ...
        $this->_set_benefactor($empl_id, $eff_date, $term_date, $text);
...
//                    )
    ...
    public function update_benefactor($edit_id, $eff_date, $term_date,$text) {
    ...
    public function set_joins() {
        ...
        $this->db->join('r_pegawai', 'r_pegawai.id_pegawai=apr_ref_acc_number.empl_id', 'left');
    }
