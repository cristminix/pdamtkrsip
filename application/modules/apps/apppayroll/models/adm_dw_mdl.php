...
class Adm_Dw_Mdl extends Apppayroll_Frontmdl {

    public $payslip_mdl_inst              = 'payslip_mdl';
    public $tbl                           = 'apr_empl_dw';
    ...
    public $rs_use_form_filter            = 'apr_empl_dw';
    ...
    protected function _set_member($empl_id, $member_since, $member_term) {
        ...
        $sqlquery    .= " AND `lock`= 0";
        $sqlquery    .= " AND `print_dt` >= '{$member_since}'";
        if ($member_term) {
            $sqlquery .= " AND `print_dt` < '{$member_term}'";
        }
        $this->db->query($sqlquery);
    }

    protected function _unset_member($empl_id, $member_since, $member_term) {
        $tbl_payslip = $this->tbl_payslip;
        $sqlquery    = "UPDATE {$tbl_payslip} ";
        $sqlquery    .= " SET ";
        $sqlquery    .= " modified=NOW()";
        $sqlquery    .= ", empl_dw=0";
        $sqlquery    .= ", ddc_dw=NULL";
        $sqlquery    .= " WHERE empl_id='{$empl_id}'";
        $sqlquery    .= " AND `lock`= 0";
        $sqlquery    .= " AND `print_dt` >= '{$member_since}'";
        if ($member_term) {
...
//        }
        ...
        if ($use_filter) {
            $this->db->where('id_pegawai NOT IN (SELECT `empl_id` FROM `apr_empl_dw` WHERE `active_status`=1) ', null, false);
        }
        $rs = $this->db->get();
        if ($rs->num_rows() <= 0) {
            return array();
    ...
    public function add_new_member($empl_id, $dw_id, $member_pos, $member_since, $member_term) {
    ...
    public function do_batch_assignment() {
        ...
        $sqlstr1 = "CREATE TEMPORARY TABLE IF NOT EXISTS tmp_dw";
...
//                        'a_class' => 'btn btn-sm btn-primary',
    ...
    public function update_member($edit_id, $dw_id, $member_pos, $member_since, $member_term) {
        ...
        $this->db->trans_begin();
        $this->_unset_member($old_detail->empl_id, $old_detail->member_since, $old_detail->member_term);
        $this->db->set('dw_id', $dw_id);
        $this->db->set('member_pos', $member_pos);
        $this->db->set('member_since', $member_since);
        if (!$member_term) {
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $this->db->trans_commit();
            $this->_set_member($old_detail->empl_id, $member_since, $member_term);
            $this->_update_payslip();
            return $id;
        }
