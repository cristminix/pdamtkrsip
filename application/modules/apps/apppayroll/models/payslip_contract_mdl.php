...
class Payslip_Contract_Mdl extends Apppayroll_Frontmdl
{
...
//        $db = $this->load->database('default', true);
    ...
    public function generate_sv_data($year, $month, $skip_check = false)
    {
        ...
        foreach ($tbls_date as $tbl => $datefield) {
            $sqlstr = Payslip_Contract_Mdl_Schema::get_resignation_id($tbl, $year, $month, $datefield);
        ...
        $sqlstr   = Payslip_Contract_Mdl_Schema::get_locked_id($tbl, $print_dt);
        $query    = $this->db->query($sqlstr);
        $rs       = $query->result();
        ...
        //
        $tbl      = $this->tbl;
        $sqlstr   = Payslip_Contract_Mdl_Schema::get_init_insert($tbl, $year, $month, $lastdate, $excl_ids_str);
        $query    = $this->db->query($sqlstr);
        //debug($sqlstr);
        // update job unit / jb title
        $tbl_join = 'r_peg_jab';
        $sqlstr   = Payslip_Contract_Mdl_Schema::get_update_job_unit_title($tbl, $tbl_join, $year, $month, $lastdate);
        $query    = $this->db->query($sqlstr);

        // update status kontrak
        $tbl_join  = 'r_peg_kontrak';
        $empl_stat = 'Kontrak';
        $sqlstr    = Payslip_Contract_Mdl_Schema::get_update_status($tbl, $tbl_join, $year, $month, $lastdate, $empl_stat);
        $query     = $this->db->query($sqlstr);
        ...
        $sqlstr   = Payslip_Contract_Mdl_Schema::get_update_work_day($tbl, $tbl_join, $year, $month, $lastdate);
        $query    = $this->db->query($sqlstr);

//        // update marital status
//        $tbl_join = 'r_peg_perkawinan';
        $tbl_join = 'apr_adm_marstat';
        $sqlstr   = Payslip_Contract_Mdl_Schema::get_update_marstat($tbl, $tbl_join, $year, $month, $lastdate);
//       debug($sqlstr);die();
        $query    = $this->db->query($sqlstr);
...
//        $tbl_join = 'r_peg_anak';
        ...
        $sqlstr   = Payslip_Contract_Mdl_Schema::get_update_child_count($tbl, $tbl_join, $year, $month, $lastdate);
        $query    = $this->db->query($sqlstr);

        // update count children fro rice alw
//        $tbl_join = 'r_peg_anak';
        $tbl_join = 'apr_adm_dependent';
        $sqlstr   = Payslip_Contract_Mdl_Schema::get_update_alw_rc_ch_cnt($tbl, $tbl_join, $year, $month, $lastdate);
//        debug($sqlstr);die();
        $query    = $this->db->query($sqlstr);

        // update grade
        $tbl_join = 'r_peg_golongan';
        $sqlstr   = Payslip_Contract_Mdl_Schema::get_update_grade($tbl, $tbl_join, $year, $month, $lastdate);
        $query    = $this->db->query($sqlstr);

        // update Base Salary
        $tbl_join = 'm_gaji_pokok';
        $sqlstr   = Payslip_Contract_Mdl_Schema::get_update_base_salary($tbl, $tbl_join, $year, $month, $lastdate);
        $query    = $this->db->query($sqlstr);

        // WC status
        $tbl_join = 'apr_empl_wc';
        $sqlstr   = Payslip_Contract_Mdl_Schema::get_update_wc($tbl, $tbl_join, $year, $month, $lastdate);
        $query    = $this->db->query($sqlstr);

        // NPWP
        $tbl_join = 'apr_adm_npwp';
        $sqlstr   = Payslip_Contract_Mdl_Schema::get_update_npwp($tbl, $tbl_join, $year, $month, $lastdate);
        $query    = $this->db->query($sqlstr);

        // FKP status
        $tbl_join = 'apr_empl_fkp';
        $sqlstr   = Payslip_Contract_Mdl_Schema::get_update_fkp($tbl, $tbl_join, $year, $month, $lastdate);
        $query    = $this->db->query($sqlstr);

        // Dharma Wanita status
        $tbl_join = 'apr_empl_dw';
        $sqlstr   = Payslip_Contract_Mdl_Schema::get_update_dw($tbl, $tbl_join, $year, $month, $lastdate);
        $query    = $this->db->query($sqlstr);

        // ZAKAT status
        $tbl_join = 'apr_empl_zk';
        $sqlstr   = Payslip_Contract_Mdl_Schema::get_update_zk($tbl, $tbl_join, $year, $month, $lastdate);
        $query    = $this->db->query($sqlstr);

        // Water Bill status
        $tbl_join = 'apr_empl_wb';
        $sqlstr   = Payslip_Contract_Mdl_Schema::get_update_wb($tbl, $tbl_join, $year, $month, $lastdate);
        $query    = $this->db->query($sqlstr);
        ...
        $sqlstr   = Payslip_Contract_Mdl_Schema::get_update_tax_netto($tbl, $year, $month, $lastdate, 'tax_netto');
        $query    = $this->db->query($sqlstr);

        // Update pph21 tarif
        $tbl_join = 'apr_ref_pph21_tariff';
        $sqlstr   = Payslip_Contract_Mdl_Schema::get_update_pph21($tbl, $tbl_join, $year, $month, $lastdate, 1, 0.05);
        $query    = $this->db->query($sqlstr);

        // Update tax annual
        $sqlstr   = Payslip_Contract_Mdl_Schema::get_update_tax_annual($tbl, $year, $month, $lastdate, 0);
        $query    = $this->db->query($sqlstr);

        // Update tax ddc ddc_pph21 alw_pph21
        $sqlstr   = Payslip_Contract_Mdl_Schema::get_update_ddc_pph21($tbl, $year, $month, $lastdate, 0);
        $query    = $this->db->query($sqlstr);
...
WHERE active_status=1 ORDER BY `menu_order` ";
    ...
    public function get_update_all_pph21($tbl, $year, $month, $lastdate)
    {
        ...
        $sqlstr   = Payslip_Contract_Mdl_Schema::get_update_ptkp($tbl, $tbl_join, $year, $month, $lastdate, 1, 54000000);
        $query    = $this->db->query($sqlstr);

        // Update Tax Deduction Components biaya_jabatan
        $tbl_join = 'apr_ref_tax_ddc';
        $sqlstr   = Payslip_Contract_Mdl_Schema::get_update_tax_comp($tbl, $tbl_join, $year, $month, $lastdate, 'tax_ddc_jt', 'biaya_jabatan', 'gross_sal');
        $query    = $this->db->query($sqlstr);

        // Update Tax Deduction Components iuran_jht
        $tbl_join = 'apr_ref_tax_ddc';
        $sqlstr   = Payslip_Contract_Mdl_Schema::get_update_tax_comp($tbl, $tbl_join, $year, $month, $lastdate, 'tax_ddc_jht', 'iuran_jht', 'base_sal');
        $query    = $this->db->query($sqlstr);

        // Update Tax Deduction Components jaminan_pensiun
        $tbl_join = 'apr_ref_tax_ddc';
        $sqlstr   = Payslip_Contract_Mdl_Schema::get_update_tax_comp($tbl, $tbl_join, $year, $month, $lastdate, 'tax_ddc_jp', 'jaminan_pensiun', 'base_sal');
        $query    = $this->db->query($sqlstr);
