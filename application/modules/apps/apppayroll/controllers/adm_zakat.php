...
/**
 * Description of adm_zakat
 *
 * @author ino
 */
class Adm_Zakat extends Apppayroll_Frontctl {

    public $main_mdl = "adm_zakat_mdl";
    ...
    protected function _do_add_new($mdl, $cur_page = 1, $per_page = 10, $order_by = null, $sort_order = 'asc', $id = null) {
        ...
        $this->{$mdl}->rs_index_where_unsigned = 'id_pegawai NOT IN (SELECT `empl_id` FROM `apr_empl_zk` WHERE `active_status`=1)';
        ...
        $this->set_page_title(lang("ZAKAT Register New"));
    ...
    protected function _do_add_new_form($mdl, $empl_id) {
        ...
        $this->set_page_title(lang("ZAKAT Registrant") . ': ' . lang('Add new'));
    ...
    protected function _do_add_new_form_post($mdl, $empl_id, $input) {
        ...
        if (!$error) {
            ...
            $do_update = $this->{$mdl}->add_new_benefactor($empl_id, $eff_date, $term_date);
            if ($do_update) {
...
//                return $error;
        ...
        $data = compact('back_url', 'detail');
        $this->set_data($data);
        $this->set_page_title(lang("ZAKAT Benefactor") . ' : ' . lang('Delete Confirmation'));
        return $this->print_page($tpl);
    }
    ...
    protected function _do_edit($mdl, $id) {
        ...
        $data = compact('rs_form_input', 'detail');
        $this->set_data($data);
        $this->set_page_title(lang("ZAKAT Registration Edit"));
        return $this->print_page($tpl);
    }
    ...
    public function edit($id = null, $cur_page = null, $per_page = 10, $order_by = null, $sort_order = 'asc') {
        ...
        $this->set_page_title(lang("ZAKAT Registration"));
