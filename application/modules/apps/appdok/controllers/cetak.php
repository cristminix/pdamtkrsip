...
class Cetak extends MX_Controller {
    ...
    if(count($data['pendidikan']) > 0){
      ...
      $html['pendidikan'] = 'Tidak Ada Data untuk ditampilkan';
		...
		$data['pengangkatan']['tetap'] = Modules::run("datamodel/pegawai/get_peg_tetap",$id_pegawai);
		$data['pengangkatan']['capeg'] = Modules::run("datamodel/pegawai/get_peg_capeg",$id_pegawai);
		...
		$data['jabatan'] = Modules::run("datamodel/pegawai/get_riwayat_jabatan",$id_pegawai);
		...
		$data['sanksi'] = Modules::run("datamodel/pegawai/get_riwayat_sanksi",$id_pegawai);
		if(count($data['sanksi']) > 0){
		  $html['sanksi'] = $this->load->view('cv/sanksi',array('data'=>$data['sanksi']),true );
		  }else{
		  $html['sanksi'] = 'Tidak Ada Data untuk ditampilkan';
		}
		
		$data['penghargaan'] = Modules::run("datamodel/pegawai/get_riwayat_penghargaan",$id_pegawai);
		if(count($data['penghargaan']) > 0){
		  ...
		  $html['penghargaan'] = 'Tidak Ada Data untuk ditampilkan';
			...
			$data['kesehatan'] = Modules::run("datamodel/pegawai/get_riwayat_kesehatan",$id_pegawai);
		if(count($data['kesehatan']) > 0){
		  $html['kesehatan'] = $this->load->view('cv/kesehatan',array('data'=>$data['kesehatan']),true );
		  }else{
		  $html['kesehatan'] = 'Tidak Ada Data untuk ditampilkan';
		}
		
    $data['psikotes'] = Modules::run("datamodel/pegawai/get_riwayat_psikotes",$id_pegawai);
		if(count($data['psikotes']) > 0){
		  $html['psikotes'] = $this->load->view('cv/psikotes',array('data'=>$data['psikotes']),true );
		  }else{
		  $html['psikotes'] = 'Tidak Ada Data untuk ditampilkan';
		}
    $data['pengalaman'] = Modules::run("datamodel/pegawai/get_riwayat_pengalaman",$id_pegawai);
		if(count($data['pengalaman']) > 0){
		  $html['pengalaman'] = $this->load->view('cv/pengalaman',array('data'=>$data['pengalaman']),true );
		  }else{
		  $html['pengalaman'] = 'Tidak Ada Data untuk ditampilkan';
		}
    $data['diklat'] = Modules::run("datamodel/pegawai/get_riwayat_diklat",$id_pegawai);
